...
import com.br.tmchickendistributor.ui.adapter.RecebimentoAdapter;
...
public class RecebimentoActivity extends AppCompatActivity implements IRecebimentoMVP.IView {
  ...
  protected void onStart() {
    ...
    txtValorTotalDevido.setText(
        FormatacaoMoeda.converterParaDolar(mPresenter.getValorTotalDevido().doubleValue()));
    ...
    try {
      adaptadorConta = new ContaAdapter(RecebimentoActivity.this, mPresenter.pesquisarContaPorId());
    } catch (Throwable throwable) {
      RecebimentoActivity.this.runOnUiThread(
          () -> {
            AbstractActivity.showToast(RecebimentoActivity.this, throwable.getMessage());
          });
    }
        ...
        new TextWatcher() {
          ...
          public void afterTextChanged(final Editable s) {
            mPresenter.setCredito(
                new BigDecimal(s.toString().isEmpty() ? 0 : cetValorAmortizar.getCurrencyDouble())
            ...
            if (mPresenter.valorTotalDevidoEhMenorOuIgualAoCredito()) {
              if (mPresenter.valorDoCreditoEhMaiorDoQueZero()) {
                ...
                AbstractActivity.showToast(mPresenter.getContext(), "Saldo Insuficiente!");
              }
  ...
  public void setOnSwtcAmortize(CompoundButton compoundButton, boolean b) {
    ...
    if (mPresenter.valorTotalDevidoEhMenorOuIgualAoCredito()) {
      ...
      if (b) {
        ...
        mPresenter.setCredito(
            new BigDecimal(cetValorAmortizar.getCurrencyDouble())
                .setScale(2, BigDecimal.ROUND_HALF_DOWN));
        mPresenter.calcularAmortizacaoAutomatica();
  ...
  public void setSpnTipoRecebimentoOnSelected(int position) {
    ...
    mPresenter.setConta(adaptadorConta.getItem(position));
  }
