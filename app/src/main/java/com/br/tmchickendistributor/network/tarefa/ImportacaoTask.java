...
public class ImportacaoTask extends AsyncTask<Void, Void, Boolean> {
    ...
    private static final String TAG = ImportacaoTask.class.getName().concat(" _______ ");
    ...
    private void salvarFuncionarios(List<Funcionario> funcionarios) {
        ...
        if (VERSION.SDK_INT >= VERSION_CODES.N) {
            funcionarios.forEach(
                    funcionario -> {
                        realm.copyToRealmOrUpdate(new FuncionarioORM(funcionario));
                    });
        } else {
        ...
        Log.d(TAG, "Foram importados " + funcionarios.size() + " funcionarios");

    }

    private void salvarClientesVendedores(List<ClienteVendedor> clientesVendedores) {
        Realm realm = Realm.getDefaultInstance();
        realm.beginTransaction();

        if (VERSION.SDK_INT >= VERSION_CODES.N) {
            clientesVendedores.forEach(
                    clienteVendedor -> {
                        ClienteVendedorID clienteVendedorID = new ClienteVendedorID();
                        clienteVendedorID.setIdCliente(clienteVendedor.getChavesClienteVendedorID().getIdCliente());
                        clienteVendedorID.setIdFuncionario(clienteVendedor.getChavesClienteVendedorID().getIdFuncionario());
                        clienteVendedor.setChavesClienteVendedorID(clienteVendedorID);
                        clienteVendedor.setId(clienteVendedorID.getIdCliente()+"-"+clienteVendedorID.getIdFuncionario());
                        realm.copyToRealmOrUpdate(new ClienteVendedorORM(clienteVendedor));
                    });
        } else {
            for (ClienteVendedor clienteVendedor : clientesVendedores) {
                ClienteVendedorID clienteVendedorID = new ClienteVendedorID();
                clienteVendedorID.setIdCliente(clienteVendedor.getChavesClienteVendedorID().getIdCliente());
                clienteVendedorID.setIdFuncionario(clienteVendedor.getChavesClienteVendedorID().getIdFuncionario());
                clienteVendedor.setChavesClienteVendedorID(clienteVendedorID);
        ...
        Log.d(TAG, "Foram importados " + clientesVendedores.size() + " clientesVendedores");
    ...
    private boolean importar() {
        ...
        try {
            ...
            if (importacaoResponse.isSuccessful()) {
                ...
                salvarFuncionarios(importacao.getFuncionarios());
                salvarClientesVendedores(importacao.getClientesVendedores());
